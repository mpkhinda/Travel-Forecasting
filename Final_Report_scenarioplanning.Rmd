...
What is the impact of a universal basic income (UBI) program on travel demand in the Oklahoma City region? We hypothesize that the total number of trips will increase. More granularly, we believe that non home-based (NHB) and home-based other (HBO) trips will increase more significantly than home-based work trips (HBW). Our original model suggests that median income has a significant, positive relationship to the number of home-based other trips. With this precedent, we expect that changing income for a large portion of the population will further increase the number of HBO trips. 

To achieve this **we proposed a $12,000 unconditional cash transfer to all residents of the MSA**; we modeled this as a naive form of UBI which has been proposed by some politicians (Andrew Yang). An unconditional cash transfer has the purpose of improving quality of life for residents. For the purposes of our study we wanted to set out to empirically test the impact of an unconditional cash transfer on travel. We hypothesize that the result will be an increase in the number of overall trips, specifically non-work-related trips. At the outset of our study we assumed that an increase in trips would result in an increase in VMT.
...
We had a number of assumptions when conducting this study: residents will not relocate given an increase in income, residents will keep the same jobs and level of employment, meaning the income is entirely supplemental, any resulting change in vehicle ownership would not be captured by our current model of productions/attractions, households will not have more children based on their change in income, households will not be more likely to move from renting to owning a home as a result of this this income, and households will not be more or less likely to have someone 65 or older living in them because of this income.
...
If time had allowed for further development of this model, we would want to better understand what happened to the VMT in our unconditional cash transfer scenario. We tried to systematically tease apart each step, but a more in depth and localized model would be a more sound approach to understanding what resulted in the combined trip number increase and VMT decrease. Additionally, more time spent on thinking through the structure of an unconditional cash transfer program would be beneficial to the realism of the modeled scenario. 
