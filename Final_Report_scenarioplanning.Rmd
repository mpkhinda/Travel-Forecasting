...
full_travel_matrix <- read_csv("full_travel_matrix.csv") %>%
  ...
  mutate(from_GEOID = as.character(from_GEOID), 
         ...
         #from_node = get_node_id(from_GEOID),
...
What is the impact of a universal basic income (UBI) program on travel demand in the Oklahoma City region? We hypothesize that the total number of trips will increase. More granularly, we believe that non home-based (NHB) and home-based other (HBO) trips will increase more significantly than home-based work trips (HBW). Our original model suggests that median income has a significant, positive relationship to the number of home-based other trips. With this precedent, we expect that changing income for a large portion of the population will further increase the number of HBO trips. Equally, we will draw on recent pilot UBI program evaluations to incorporate the effect of additional income on rates of car ownership, employment, and other household characteristics.

To achieve this **we proposed a $12,000 unconditional cash transfer to all residents of the MSA**; modeled as a naive form of UBI which has been proposed by some politicians (Andrew Yang). An unconditional cash transfer has the purpose of improving quality of life for residents. For the purposes of our study we wanted to set out to empirically test the impact an unconditional cash transfer would have on travel. We hypothesize that the result will be an increase in the number of overall trips, specifically non-work-related trips. At the outset of our study we assumed that increase in trips would result in an increase in VMT.

To test this we changed median income to reflect the $12,000 cash transfer; reran a linear regression in the trip generation stage; generated a new number of productions and attractions, and compared this to the number of trips (and share of trips by purpose) in the original model (we performed this same comparison for the number of VMT); and finally, we reran trip distribution, mode choice, trip assignment.  

We had a number of assumptions when conducting this study: residents will not relocate given an increase in income, residents will keep the same jobs and level of employment, meaning the income is entirely supplemental, any resulting change in vehicle ownership would not be captured by our current model of productions/attractions, households will not have more children based on their change in income, households will not be more likely to move from renting to owning a home as a result of this this income, households will not be more or less likely to have someone 65 or older living in them because of this income.
...
The Oklahoma City MSA is predominantly white, with 64.3% of residents identifying as white alone, followed by 13.3% Hispanic or Latino, 10.1% Black, 3.3% American Indian and Alaska Native, 3.1% Asian, 0.1% Native Hawaiian and Pacific Islander. 36.8% of residents in the study are are ages 35-64, 24.7% are under the age of 18, 25.0% are ages 18-34, and 13.6% are ages 65 and older. The median household income in 2021 was $59,084 – higher than the statewide average and lower than the national average. About 13.9% of residents live below the poverty line (Figure 1) – lower than the statewide average and higher than the national average.

```{r poverty-line, echo=FALSE, warning=FALSE, fig.cap='Figure 1. Households Living Below the Federal Poverty Line are Concentrated Near Downtown Oklahoma City', out.width='80%', fig.asp=.75, fig.align='center'}
...
* We assume that employment exists in three categories (Figure 2):
    * Retail
    * Service
    * Basic 
...
```{r employment-county, echo=FALSE, fig.cap='Figure 2. Oklahoma County has the Most Employees in the MSA, and the Majority Are Service Sector Workers', out.width='90%', fig.asp=.75, fig.align='center'}
...
OKC generally follows a monocentric pattern of development with the greatest density of people living and working in downtown Oklahoma City (Figures 3-4), located in the center of the MSA.
...
```{r emp-density, echo=FALSE, fig.cap='Figure 3. Population Density is Greatest in the Center of the Oklahoma City MSA', out.width='80%', fig.asp=.75, fig.align='center'}
...
```{r pop-density, echo=FALSE, fig.cap='Figure 4. Job Density is Also Concentrated in the Center of the Oklahoma City MSA', out.width='80%', fig.asp=.75, fig.align='center'}
...
``` {r access-indices, echo=FALSE, fig.cap='Figure 6. The Transit Hub South of Downtown Oklahoma City is A Recognizable Blip on this Plot Showing Car and Transit Accessibility Plotted Against One Another', out.width='80%', fig.asp=.75, fig.align='center'}
...
```{r access-histogram, echo=FALSE, fig.cap='Figure 7. Nearly Two-Thirds of Zones Do Not Have Any Transit Accessibility', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(accessibility_summary) +
...
```{r spatial-access-transit, echo=FALSE, warning=FALSE, fig.cap='Figure 9. Accessibility by Transit is Concentrated in the Downtown Area, with Some Accessibility around a Southern Transit Hub in Norman', out.width='80%', fig.asp=.75, fig.align='center'}
...
```{r centroids_zones, echo=FALSE, fig.cap='Figure 10. Components of our Model: Zones and their Centroids', out.width='80%', fig.asp=.75, fig.align='center'}
county_geom <- full_tract_info %>%
    separate(NAME, into = c("tract-num", "county", "state"), sep = ", ") %>%
    group_by(county) %>%
    summarize(geometry = st_union(geometry))
...
```{r roads, echo=FALSE, fig.cap='Figure 11. Components of our Model:Roads, Centroids, Centroid Connectors, and Transit Lines', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot() +
  annotation_map_tile(zoomin = 0, progress = "none", type = "cartolight") +
...
```{r transit, echo=FALSE, fig.cap='Figure 12. Components of our Model:Roads, Centroids, Centroid Connectors, and Transit Lines', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot() +
  annotation_map_tile(zoomin = 0, progress = "none", type = "cartolight") +
  geom_sf(data = full_tract_info, aes(), fill = NA, color="darkgrey", size=0.1) +
  geom_sf(data = county_geom, aes(), fill = NA, color="black", size=0.5) +
  geom_sf(data = transit_geom, aes(color = shape_id), show.legend = FALSE) +
  geom_sf_label(data = county_geom, aes(label = county), alpha = 0, label.size = NA, size =4, fontface = "bold") +
  annotation_scale(location = 'br', text_cex = 0.5) +
  labs(x = "", 
       y = "") +
...
The charts below illustrate how the timing of various transit trips break down across walk, wait, transfer and travel time (Figures 13-14).


```{r stacked-bar-chart, echo=FALSE, warning=FALSE, fig.cap='Figures 13 and 14. Travel Time Makes Up the Bulk of Most Trips, but Transfer Wait Time is Also A Large Portion of Trip Duration', out.width='80%', fig.asp=.75, fig.align='center'}
...
ggplot(temp_matrix_stacked, aes(x = reorder(to_GEOID, value), y = value, fill = variable)) + 
  ...
  labs(y = "Number of Minutes", x = " ", fill = "Public Transportation Activity") + 
    labs(subtitle = paste("Transit Time to all zones accessible from", iso_origin, "(minutes)")) +
  theme(legend.position = "bottom", axis.text.x=element_blank(), axis.ticks.x=element_blank(), panel.background = element_rect(fill = "white"))
}
...
As expected, productions and attractions are primarily concentrated in the downtown area—attractions even more so than productions (Figures 15-16). Both generally follow the density maps of residences and employment that we created earlier in the semester.


```{r density-prod, echo=FALSE, fig.cap='Figure 15. Trip Productions are Primarily Concentrated in the Downtown Area', out.width='80%', fig.asp=.75, fig.align='center'}
...
```{r density-attr, echo=FALSE, fig.cap='Figure 16. Trip Attractions are Even More Concentrated in the Downtown Area', out.width='80%', fig.asp=.75, fig.align='center'}
#attractions
...
# calculate friction factors (using exponential function)
...
```{r hbw-chord, echo = FALSE, fig.cap='Figure 18. Intra- and Inter-County Flows for Home-Based Work Trips', fig.align='center'}
...
```{r nhb-chord, echo = FALSE, fig.cap='Figure 19. Intra- and Inter-County Flows for Non-Home-Based Trips', fig.align='center'}
chorddiag(nhb_mat, groupColors = brewer.pal(7, "Set2"), groupnamePadding = 50, tickInterval = 10000)
```


### Scenario B: Unconditional Cash Transfer


We used the same variables in our linear regressions when running the trip generation and distribution step for our unconditional cash transfer scenario: median income (now increased by $12,000), presence of children, homeownership, and presence of someone 65 or older.

```{r calculate trip productions with UBI, echo=FALSE}
tract_trip_info_UBI <- full_tract_info %>%
  mutate(med_inc_UBI = med_incE + 12000)
  
  
tract_trip_info_UBI <- tract_trip_info_UBI %>%  
  mutate(hbo_prod_per_hh = hbo_trip_prod$coefficients["(Intercept)"] + 
                           hbo_trip_prod$coefficients["log2(inc_k)"] * log2(med_inc_UBI) + 
                           hbo_trip_prod$coefficients["any_kidsTRUE"] *  (hhs_with_kidsE/total_hhsE) + 
                           hbo_trip_prod$coefficients["homeownershipTRUE"] * (owner_occE / total_hhsE) +
                           hbo_trip_prod$coefficients["age_65TRUE"] * (hh_65plusE / total_hhsE)) %>%
  mutate(hbo_trip_prod = total_hhsE * hbo_prod_per_hh)
...
tract_trip_info_UBI <- tract_trip_info_UBI %>%
  mutate(hbw_prod_per_hh = hbw_trip_prod$coefficients["(Intercept)"] + 
                           hbw_trip_prod$coefficients["log2(inc_k)"] * log2(med_inc_UBI) + 
                           hbw_trip_prod$coefficients["any_kidsTRUE"] *  (hhs_with_kidsE/total_hhsE) + 
                           hbw_trip_prod$coefficients["homeownershipTRUE"] * (owner_occE / total_hhsE) +
                           hbw_trip_prod$coefficients["age_65TRUE"] * (hh_65plusE / total_hhsE)) %>%
  mutate(hbw_trip_prod = total_hhsE * hbw_prod_per_hh)
...
tract_trip_info_UBI <- tract_trip_info_UBI %>%
  mutate(nhb_prod_per_hh = nhb_trip_prod$coefficients["(Intercept)"] + 
                           nhb_trip_prod$coefficients["log2(inc_k)"] * log2(med_inc_UBI) + 
                           nhb_trip_prod$coefficients["any_kidsTRUE"] *  (hhs_with_kidsE/total_hhsE) + 
                           nhb_trip_prod$coefficients["homeownershipTRUE"] * (owner_occE / total_hhsE) +
                           nhb_trip_prod$coefficients["age_65TRUE"] * (hh_65plusE / total_hhsE)) %>%
  mutate(nhb_trip_prod = total_hhsE * nhb_prod_per_hh)
...
```{r trip attractions with UBI, echo=FALSE}
tract_trip_info_UBI <- tract_trip_info_UBI %>%
  mutate(hbo_trip_attr = 1.0 * total_hhsE +
                         0.3 * basic_emp +
                         5.9 * retail_emp +
...
```{r comparing productions and attractions with UBI, include=FALSE}
sum(tract_trip_info_UBI$hbo_trip_prod)

sum(tract_trip_info_UBI$hbo_trip_attr)

tract_trip_info_UBI <- tract_trip_info_UBI %>%
  mutate(hbo_bal_attr = hbo_trip_attr * 
           sum(hbo_trip_prod) / sum(hbo_trip_attr))

sum(tract_trip_info_UBI$hbo_bal_attr)


sum(tract_trip_info_UBI$hbw_trip_prod)

sum(tract_trip_info_UBI$hbw_trip_attr)

tract_trip_info_UBI <- tract_trip_info_UBI %>%
  mutate(hbw_bal_attr = hbw_trip_attr * 
           sum(hbw_trip_prod) / sum(hbw_trip_attr))

sum(tract_trip_info_UBI$hbw_bal_attr)

sum(tract_trip_info_UBI$nhb_trip_prod)

sum(tract_trip_info_UBI$nhb_trip_attr)

tract_trip_info_UBI <- tract_trip_info_UBI %>%
  mutate(nhb_bal_attr = nhb_trip_attr * 
           sum(nhb_trip_prod) / sum(nhb_trip_attr))

sum(tract_trip_info_UBI$nhb_bal_attr)
```

```{r df showing difference in new scenario, echo=FALSE}
temp_UBI <- tract_trip_info_UBI %>%
  select(GEOID, hbo_trip_prod, hbw_trip_prod, nhb_trip_prod, hbo_trip_attr, hbw_trip_attr, nhb_trip_attr) %>%
  rename(hbo_trip_prod_UBI = hbo_trip_prod,
         hbw_trip_prod_UBI = hbw_trip_prod, 
         nhb_trip_prod_UBI = nhb_trip_prod, 
         hbo_trip_attr_UBI = hbo_trip_attr, 
         hbw_trip_attr_UBI = hbw_trip_attr, 
         nhb_trip_attr_UBI = nhb_trip_attr)

difference_df <- tract_trip_info %>%
  select(GEOID, hbo_trip_prod, hbw_trip_prod, nhb_trip_prod, hbo_trip_attr, hbw_trip_attr, nhb_trip_attr) %>%
  left_join((temp_UBI %>% st_drop_geometry()))
```
```{r mutate difference columns, echo=FALSE}
difference_df <- difference_df %>%
  mutate(diff_hbo = hbo_trip_prod_UBI - hbo_trip_prod,
         diff_hbw = hbw_trip_prod_UBI - hbw_trip_prod,
         diff_nhb = nhb_trip_prod_UBI - nhb_trip_prod)
```


Compared to the number of trip productions generated in the model reflecting existing conditions, the model with an unconditional cash transfer had slightly greater trip productions for all trip types (Figures 20-22). The tracts with the greatest increases in trip productions are not particularly geographically concentrated. 


```{r plot hbo difference, echo=FALSE, fig.cap='Figure 20. Difference in Number of Home-Based Other Trip Productions in Scenario B compared to Scenario A', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(difference_df) +
  geom_sf(aes(fill=diff_hbo)) +
  scale_fill_continuous(name = "Number of Trip Productions") + 
  theme_void()
```

```{r plot hbw difference, echo=FALSE, fig.cap='Figure 21. Difference in Number of Home-Based Work Trip Productions in Scenario B compared to Scenario A', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(difference_df) +
  geom_sf(aes(fill=diff_hbw)) +
  scale_fill_continuous(name = "Number of Trip Productions") + 
  theme_void()
```

```{r plot nhb difference, echo=FALSE, fig.cap='Figure 22. Difference in Number of Non-Home-Based Trip Productions in Scenario B compared to Scenario A', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(difference_df) +
  geom_sf(aes(fill=diff_nhb)) +
  scale_fill_continuous(name = "Number of Trip Productions") + 
  theme_void()
```


```{r estimate travel flows UBI, echo = FALSE}
full_TTmatrix_UBI <- full_TTmatrix %>%
  mutate(F_HBO_UBI = exp(-0.18 * drive_time),
         F_HBW_UBI = exp(-0.06 * drive_time),
         F_NHB_UBI = exp(-0.19 * drive_time))

HBO_dist_UBI <- grvty_balancing(od_zones = tract_trip_info_UBI,
                            friction = full_TTmatrix_UBI,
                            zone_id = "GEOID",
                            zone_o = "hbo_trip_prod",
                            zone_d = "hbo_bal_attr",
                            friction_o_id = "from_GEOID",
                            friction_d_id = "to_GEOID",
                            friction_factor = "F_HBO_UBI",
                            tolerance = 0.01,
                            max_iter = 50000)

HBW_dist_UBI <- grvty_balancing(od_zones = tract_trip_info_UBI,
                            friction = full_TTmatrix_UBI,
                            zone_id = "GEOID",
                            zone_o = "hbw_trip_prod",
                            zone_d = "hbw_bal_attr",
                            friction_o_id = "from_GEOID",
                            friction_d_id = "to_GEOID",
                            friction_factor = "F_HBW_UBI",
                            tolerance = 0.01,
                            max_iter = 50000)

NHB_dist_UBI <- grvty_balancing(od_zones = tract_trip_info_UBI,
                            friction = full_TTmatrix_UBI,
                            zone_id = "GEOID",
                            zone_o = "nhb_trip_prod",
                            zone_d = "nhb_bal_attr",
                            friction_o_id = "from_GEOID",
                            friction_d_id = "to_GEOID",
                            friction_factor = "F_NHB_UBI",
                            tolerance = 0.01,
                            max_iter = 50000)

HBO_flows_UBI <- HBO_dist_UBI$flows %>%
  rename(from_GEOID = o_id,
         to_GEOID = d_id,
         HBO_flow_UBI = flow)

HBW_flows_UBI <- HBW_dist_UBI$flows %>%
  rename(from_GEOID = o_id,
         to_GEOID = d_id,
         HBW_flow_UBI = flow)

NHB_flows_UBI <- NHB_dist_UBI$flows %>%
  rename(from_GEOID = o_id,
         to_GEOID = d_id,
         NHB_flow_UBI = flow)

full_TTmatrix_UBI <- full_TTmatrix_UBI %>%
  left_join(HBO_flows_UBI) %>%
  left_join(HBW_flows_UBI) %>%
  left_join(NHB_flows_UBI)

# Check avg travel times by purpose (compare with NHTS avg)
hbo_est_UBI <- sum(full_TTmatrix_UBI$HBO_flow_UBI * full_TTmatrix_UBI$drive_time) / sum(full_TTmatrix_UBI$HBO_flow_UBI)
hbw_est_UBI <- sum(full_TTmatrix_UBI$HBW_flow_UBI * full_TTmatrix_UBI$drive_time) / sum(full_TTmatrix_UBI$HBW_flow_UBI)
nhb_est_UBI <- sum(full_TTmatrix_UBI$NHB_flow_UBI * full_TTmatrix_UBI$drive_time) / sum(full_TTmatrix_UBI$NHB_flow_UBI)

ttime_by_purpose_pred_UBI <- ttime_by_purpose %>%
  mutate(pred_time_UBI = c(hbo_est_UBI, hbw_est_UBI, nhb_est_UBI)) %>%
  mutate(time_dif_UBI = avg_time - pred_time_UBI) %>%
  rename(nhts_avg_time = avg_time) %>%
  select(-avg_time_se)

kable(ttime_by_purpose_pred_UBI)
```

For the friction factors for the gravity balancing functions, we initially tried the same variable values as those we used in the original model. This resulted in estimated average trip times that were within the 0.2 minute threshold for HBO and HBW trips, but not for NHB trips. We adjusted the value down slightly, from -0.22 to -0.19, to be within the threshold.


### Visualizing Travel Flows by Purpose and County

Trip flows within and between counties remained fairly consistent across the two scenarios (Figures 23-25). 


```{r chord diagram UBI, echo = FALSE, message = FALSE}
county_skim_UBI <- full_TTmatrix_UBI %>%
  mutate(from_county = substr(from_GEOID, 1, 5),
         to_county = substr(to_GEOID, 1, 5)) %>%
  group_by(from_county, to_county) %>%
  summarise(HBO_flow_UBI = sum(HBO_flow_UBI), 
            HBW_flow_UBI = sum(HBW_flow_UBI), 
            NHB_flow_UBI = sum(NHB_flow_UBI)) %>%
  filter(HBO_flow_UBI > 0) %>%
  filter(HBW_flow_UBI > 0) %>%
  filter(NHB_flow_UBI > 0)

labeled_skim_UBI <- tibble(prod_name = sort(rep(county_names, 6)),
                       attr_name = rep(county_names, 6))

labeled_skim_UBI <- labeled_skim_UBI %>%
  mutate(from_county = case_when(
                         prod_name == "Oklahoma" ~ "40109",
                         prod_name == "Cleveland" ~ "40027",
                         prod_name == "Canadian" ~ "40017",
         ...
         to_county = case_when(
                         ...
                         attr_name == "Logan" ~ "40083",
                         attr_name == "McClain" ~ "40087")) %>%
  left_join(county_skim_UBI) %>%
  replace_na(list(HBO_flow_UBI = 0)) %>%
  replace_na(list(HBW_flow_UBI = 0)) %>%
  replace_na(list(NHB_flow_UBI = 0))

hbo_mat_UBI <- matrix(labeled_skim_UBI$HBO_flow_UBI,
                  byrow = TRUE,
                  nrow = 7, ncol = 7)

dimnames(hbo_mat_UBI) <- list(production = county_names,
                          attraction = county_names)

hbw_mat_UBI <- matrix(labeled_skim_UBI$HBW_flow_UBI,
                  byrow = TRUE,
                  nrow = 7, ncol = 7)

dimnames(hbw_mat_UBI) <- list(production = county_names,
                          attraction = county_names)

nhb_mat_UBI <- matrix(labeled_skim_UBI$NHB_flow_UBI,
                  byrow = TRUE,
                  nrow = 7, ncol = 7)

dimnames(nhb_mat_UBI) <- list(production = county_names,
                          attraction = county_names)
```
...
```{r hbo-chord-UBI, echo = FALSE, fig.cap='Figure 23. Intra- and Inter-County Flows for Home-Based Other Trips', fig.align='center'}
chorddiag(hbo_mat_UBI, groupColors = brewer.pal(7, "Set2"), groupnamePadding = 50, tickInterval = 50000)
```
...
```{r hbw-chord-UBI, echo = FALSE, fig.cap='Figure 24. Intra- and Inter-County Flows for Home-Based Work Trips', fig.align='center'}
chorddiag(hbw_mat_UBI, groupColors = brewer.pal(7, "Set2"), groupnamePadding = 50, tickInterval = 10000)
```


##### Non-Home Based Trips by County

```{r nhb-chord-UBI, echo = FALSE, fig.cap='Figure 25. Intra- and Inter-County Flows for Non-Home-Based Trips', fig.align='center'}
chorddiag(nhb_mat_UBI, groupColors = brewer.pal(7, "Set2"), groupnamePadding = 50, tickInterval = 10000)
```
...
# calculate drive costs by mile using IRS rate
full_travel_matrix <- full_travel_matrix %>%
...
In order to understand the relationship between trip length and mode choice, we plotted the percent of trips using a particular mode (as predicted by our model) against the drive time of that trip (Figure 26). We then further subset the data by purpose to illustrate the difference in mode share for different kinds of trips (Figures 27-29). 
...
```{r mode-split-all, fig.cap='Figure 26. Mode Split by Drive Time for all Trip Purposes', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot()+
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_transit_total/n_total_total), color = "Transit"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_SOV_total/n_total_total), color = "SOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_HOV_total/n_total_total), color = "HOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  scale_x_continuous(breaks = seq(from =0, to = 240, by = 30)) +
  labs(x = "Drive Time (mins)", y = "Mode Share (Fraction of Total Trips)", color = "Mode") +
  theme_minimal()
```

```{r mode-split-hbo, fig.cap='Figure 27. Mode Split by Drive Time for Home-Based Other Trips', out.width='80%', fig.asp=.75, fig.align='center'}
# HBO
ggplot()+
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_transit_HBO/n_total_HBO), color = "Transit"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_SOV_HBO/n_total_HBO), color = "SOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_HOV_HBO/n_total_HBO), color = "HOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  scale_x_continuous(breaks = seq(from =0, to = 240, by = 30)) +
  labs(title = "Mode Split by Drive Time for Home-Based Other Trips", x = "Drive Time (mins)", y = "Mode Share (Fraction of Total Trips)", color = "Mode") +
  theme_minimal()
```

```{r mode-split-hbw, fig.cap='Figure 28. Mode Split by Drive Time for Home-Based Work Trips', out.width='80%', fig.asp=.75, fig.align='center'}
# HBW
ggplot()+
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_transit_HBW/n_total_HBW), color = "Transit"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_SOV_HBW/n_total_HBW), color = "SOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_HOV_HBW/n_total_HBW), color = "HOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  scale_x_continuous(breaks = seq(from =0, to = 240, by = 30)) +
  labs(title = "Mode Split by Drive Time for Home-Based Work Trips", x = "Drive Time (mins)", y = "Mode Share (Fraction of Total Trips)", color = "Mode") +
  theme_minimal()
```

```{r mode-split-nhb, fig.cap='Figure 29. Mode Split by Drive Time for Non-Home-Based Trips', out.width='80%', fig.asp=.75, fig.align='center'}
# NHB
ggplot()+
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_transit_NHB/n_total_NHB), color = "Transit"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_SOV_NHB/n_total_NHB), color = "SOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_travel_matrix, aes(x = drive_time, y = (n_HOV_NHB/n_total_NHB), color = "HOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  scale_x_continuous(breaks = seq(from =0, to = 240, by = 30)) +
  labs(title = "Mode Split by Drive Time for Non Home-Based Trips", x = "Drive Time (mins)", y = "Mode Share (Fraction of Total Trips)", color = "Mode") +
  theme_minimal()
...
We used the same assumptions for travel cost in our unconditional cash transfer model: we used linear distance and the 2023 IRS mileage reimbursement rate (0.655 dollars) to calculate trip costs; we established carpool costs using the “Carpool 2+ only—daily” assumptions from NCHRP 716; for transit costs, we estimated an average of the Embark system’s different fares, assigning each trip a $2.00 fare with free transfers. 


#### Model Coefficients + Calibration 

We compared mode split by trip type to the same observed data from NHTS as we used in Scenario A. We started by assigning the coefficients from our Scenario A mode choice model to the Scenario B mode choice model. We applied the same standard for Scenario B, as well, aiming to have any discrepancies between the model and observed data from NHTS below a 5 percentage point threshold. 

HBO: SOV = -0.28, HOV = -0.25, transit = -3.52
HBW: SOV = 0.70, -2.00, transit = -1.00
NHB: original constants (log odds of that particular mode for that purpose from the NHTS)

```{r constant calcs UBI, echo=FALSE, include=FALSE}
# HBO Model
#SOV_share_HBO_UBI <- 
#  mode_by_purpose$pct_SOV[mode_by_purpose$purpose == "HBO"]
...
#transit_share_NHB_UBI <- 
#  mode_by_purpose$pct_transit[mode_by_purpose$purpose == "NHB"]


# CONSTANTS
SOV_const_HBO_UBI <- SOV_const_HBO
HOV_const_HBO_UBI <- HOV_const_HBO
transit_const_HBO_UBI <- transit_const_HBO

SOV_const_HBW_UBI <- SOV_const_HBW
HOV_const_HBW_UBI <- HOV_const_HBW
transit_const_HBW_UBI <- transit_const_HBW

SOV_const_NHB_UBI <- SOV_const_NHB
HOV_const_NHB_UBI <- HOV_const_NHB
transit_const_NHB_UBI <- transit_const_NHB
```


```{r mode choice discrete model UBI, echo=FALSE}
full_TTmatrix_UBI <- full_TTmatrix_UBI %>%
  mutate(utility_transit_HBO_UBI = transit_const_HBO_UBI +
                               ivtt * -0.010  +
                               ovtt * -0.046 +
                               fare * -0.029,
         utility_SOV_HBO_UBI = SOV_const_HBO_UBI +
                           drive_time * -0.010 +
                           drive_cost * -0.029,
         utility_HOV_HBO_UBI = HOV_const_HBO_UBI +
                           drive_time * -0.010 +
                           carpool_cost_hbo * -0.029) %>%
  mutate(exp_u_SOV_HBO_UBI = exp(utility_SOV_HBO_UBI),
         exp_u_HOV_HBO_UBI = exp(utility_HOV_HBO_UBI),
         exp_u_transit_HBO_UBI = exp(utility_transit_HBO_UBI)) %>%
  rowwise() %>%
  mutate(total_utility_HBO_UBI = sum(exp_u_SOV_HBO_UBI,
                                 exp_u_HOV_HBO_UBI,
                                 exp_u_transit_HBO_UBI,
                                 na.rm = TRUE)) %>%
  ungroup()

# Calculate utility of HBW trips (B model coef)
full_TTmatrix_UBI <- full_TTmatrix_UBI %>%
  mutate(utility_transit_HBW_UBI = transit_const_HBW_UBI +
                               ivtt * -0.030  +
                               ovtt * -0.075 +
                               fare * -0.0043,
         utility_SOV_HBW_UBI = SOV_const_HBW_UBI +
                           drive_time * -0.030 +
                           drive_cost * -0.0043,
         utility_HOV_HBW_UBI = HOV_const_HBW_UBI +
                           drive_time * -0.030 +
                           carpool_cost_hbw * -0.0043) %>%
  mutate(exp_u_SOV_HBW_UBI = exp(utility_SOV_HBW_UBI),
         exp_u_HOV_HBW_UBI = exp(utility_HOV_HBW_UBI),
         exp_u_transit_HBW_UBI = exp(utility_transit_HBW_UBI)) %>%
  rowwise() %>%
  mutate(total_utility_HBW_UBI = sum(exp_u_SOV_HBW_UBI,
                                 exp_u_HOV_HBW_UBI,
                                 exp_u_transit_HBW_UBI,
                                 na.rm = TRUE)) %>%
  ungroup()

# Calculate utility of NHB trips (D model coef)
full_TTmatrix_UBI <- full_TTmatrix_UBI %>%
  mutate(utility_transit_NHB_UBI = transit_const_NHB_UBI +
                               ivtt * -0.011  +
                               (access_wt + egress_wt) * -0.066 +
                               init_wt * -0.061 +
                               transfer_wt * -0.059 +
                               fare * -0.033,
         utility_SOV_NHB_UBI = SOV_const_NHB_UBI +
                           drive_time * -0.011 +
                           drive_cost * -0.033,
         utility_HOV_NHB_UBI = HOV_const_NHB_UBI +
                           drive_time * -0.011 +
                           carpool_cost_nhb * -0.033) %>%
  mutate(exp_u_SOV_NHB_UBI = exp(utility_SOV_NHB_UBI),
         exp_u_HOV_NHB_UBI = exp(utility_HOV_NHB_UBI),
         exp_u_transit_NHB_UBI = exp(utility_transit_NHB_UBI)) %>%
  rowwise() %>%
  mutate(total_utility_NHB_UBI = sum(exp_u_SOV_NHB_UBI,
                                 exp_u_HOV_NHB_UBI,
                                 exp_u_transit_NHB_UBI,
                                 na.rm = TRUE)) %>%
  ungroup()
...
```{r probability mode share 2 UBI, echo=FALSE, include=FALSE}
# determine mode share for HBO
full_TTmatrix_UBI <- full_TTmatrix_UBI %>%
  mutate(p_transit_HBO_UBI = exp(utility_transit_HBO_UBI) / total_utility_HBO_UBI,
         p_SOV_HBO_UBI = exp(utility_SOV_HBO_UBI) / total_utility_HBO_UBI,
         p_HOV_HBO_UBI = exp(utility_HOV_HBO_UBI) / total_utility_HBO_UBI) %>%
  replace_na(list(p_transit_HBO_UBI = 0,
                  p_SOV_HBO_UBI = 0,
                  p_HOV_HBO_UBI = 0))

# determine mode share for HBW
full_TTmatrix_UBI <- full_TTmatrix_UBI %>%
  mutate(p_transit_HBW_UBI = exp(utility_transit_HBW_UBI) / total_utility_HBW_UBI,
         p_SOV_HBW_UBI = exp(utility_SOV_HBW_UBI) / total_utility_HBW_UBI,
         p_HOV_HBW_UBI = exp(utility_HOV_HBW_UBI) / total_utility_HBW_UBI) %>%
  replace_na(list(p_transit_HBW_UBI = 0,
                  p_SOV_HBW_UBI = 0,
                  p_HOV_HBW_UBI = 0))

# determine mode share for NHB
full_TTmatrix_UBI <- full_TTmatrix_UBI %>%
  mutate(p_transit_NHB_UBI = exp(utility_transit_NHB_UBI) / total_utility_NHB_UBI,
         p_SOV_NHB_UBI = exp(utility_SOV_NHB_UBI) / total_utility_NHB_UBI,
         p_HOV_NHB_UBI = exp(utility_HOV_NHB_UBI) / total_utility_NHB_UBI) %>%
  replace_na(list(p_transit_NHB_UBI = 0,
                  p_SOV_NHB_UBI = 0,
                  p_HOV_NHB_UBI = 0))
```

```{r assign trips 2 UBI, echo=FALSE, include = FALSE}
full_TTmatrix_UBI <- full_TTmatrix_UBI %>%
  mutate(n_transit_HBO_UBI = round(HBO_flow_UBI * p_transit_HBO_UBI),
         n_SOV_HBO_UBI = round(HBO_flow_UBI * p_SOV_HBO_UBI),
         n_HOV_HBO_UBI = round(HBO_flow_UBI * p_HOV_HBO_UBI),
         n_transit_HBW_UBI = round(HBW_flow_UBI * p_transit_HBW_UBI),
         n_SOV_HBW_UBI = round(HBW_flow_UBI * p_SOV_HBW_UBI),
         n_HOV_HBW_UBI = round(HBW_flow_UBI * p_HOV_HBW_UBI),
         n_transit_NHB_UBI = round(NHB_flow_UBI * p_transit_NHB_UBI),
         n_SOV_NHB_UBI = round(NHB_flow_UBI * p_SOV_NHB_UBI),
         n_HOV_NHB_UBI = round(NHB_flow_UBI * p_HOV_NHB_UBI)) %>%
  replace_na(list(n_transit_HBO_UBI = 0,
                  n_SOV_HBO_UBI = 0,
                  n_HOV_HBO_UBI = 0,
                  n_transit_HBW_UBI = 0,
                  n_SOV_HBW_UBI = 0,
...
```{r compare model to nhts 2 UBI, echo=FALSE}
modeled_mode_by_purpose_UBI <- tibble(
  purpose = c("HBO_model_UBI", "HBW_model_UBI", "NHB_model_UBI"), 
  pct_SOV = c(sum(full_TTmatrix_UBI$n_SOV_HBO_UBI) / 
                sum(full_TTmatrix_UBI$HBO_flow_UBI),
              sum(full_TTmatrix_UBI$n_SOV_HBW_UBI) / 
                sum(full_TTmatrix_UBI$HBW_flow_UBI),
              sum(full_TTmatrix_UBI$n_SOV_NHB_UBI) / 
                   sum(full_TTmatrix_UBI$NHB_flow_UBI)),
  pct_HOV = c(sum(full_TTmatrix_UBI$n_HOV_HBO_UBI) / 
                sum(full_TTmatrix_UBI$HBO_flow_UBI),
              sum(full_TTmatrix_UBI$n_HOV_HBW_UBI) / 
                sum(full_TTmatrix_UBI$HBW_flow_UBI),
              sum(full_TTmatrix_UBI$n_HOV_NHB_UBI) / 
                   sum(full_TTmatrix_UBI$NHB_flow_UBI)),
  pct_transit = c(sum(full_TTmatrix_UBI$n_transit_HBO_UBI) / 
                   sum(full_TTmatrix_UBI$HBO_flow_UBI),
                 sum(full_TTmatrix_UBI$n_transit_HBW_UBI) / 
                   sum(full_TTmatrix_UBI$HBW_flow_UBI),
                sum(full_TTmatrix_UBI$n_transit_NHB_UBI) / 
                   sum(full_TTmatrix_UBI$NHB_flow_UBI)))

model_compare_UBI <- rbind(mode_by_purpose, modeled_mode_by_purpose_UBI) %>%
  arrange(purpose) 

kable(model_compare_UBI)
```

The differences between the Scenario B modeled mode shares and the NHTS data were within the 5 percentage point threshold on the first run of our model, so no further calibration was necessary. This aligns with expectations that mode choice would not significantly change due to a slight increase in trip productions.


#### Visualizations


This visualizations further confirm our expectation that mode choice is not significantly affected by an additional influx of income (Figures 30-33). 


```{r total trip count UBI, echo=FALSE, include=FALSE}
full_TTmatrix_UBI <- full_TTmatrix_UBI %>%
  rowwise() %>%
  mutate(n_total_total_UBI = sum(n_transit_HBO_UBI,
                             n_SOV_HBO_UBI,
                             n_HOV_HBO_UBI,
                             n_transit_HBW_UBI,
                             n_SOV_HBW_UBI,
                             ...
                             n_SOV_NHB_UBI,
                             n_HOV_NHB_UBI),
           n_transit_total_UBI = sum(n_transit_HBO_UBI,
                                 n_transit_HBW_UBI,
                                 n_transit_NHB_UBI),
           n_SOV_total_UBI = sum(n_SOV_HBO_UBI,
                             n_SOV_HBW_UBI,
                             n_SOV_NHB_UBI),
           n_HOV_total_UBI = sum(n_HOV_HBO_UBI,
                             n_HOV_HBW_UBI,
                             n_HOV_NHB_UBI),
           n_total_HBO_UBI = sum(n_transit_HBO_UBI,
                               n_SOV_HBO_UBI,
                               n_HOV_HBO_UBI),
           n_total_HBW_UBI = sum(n_transit_HBW_UBI,
                               n_SOV_HBW_UBI,
                               n_HOV_HBW_UBI),
           n_total_NHB_UBI = sum(n_transit_NHB_UBI,
                               n_SOV_NHB_UBI,
                               n_HOV_NHB_UBI))
```


```{r mode-split-all-ubi, fig.cap='Figure 30. Mode Split by Drive Time for all Trip Purposes, in an Unconditional Cash Flow Scenario', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot()+
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_transit_total_UBI/n_total_total_UBI), color = "Transit"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_SOV_total_UBI/n_total_total_UBI), color = "SOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_HOV_total_UBI/n_total_total_UBI), color = "HOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  scale_x_continuous(breaks = seq(from =0, to = 240, by = 30)) +
  labs(x = "Drive Time (mins)", y = "Mode Share (Fraction of Total Trips)", color = "Mode") +
  theme_minimal()
```

```{r mode-split-hbo-ubi, fig.cap='Figure 31. Mode Split by Drive Time for Home-Based Other Trips, in an Unconditional Cash Flow Scenario', out.width='80%', fig.asp=.75, fig.align='center'}
# HBO
ggplot()+
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_transit_HBO_UBI/n_total_HBO_UBI), color = "Transit"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_SOV_HBO_UBI/n_total_HBO_UBI), color = "SOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_HOV_HBO_UBI/n_total_HBO_UBI), color = "HOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  scale_x_continuous(breaks = seq(from =0, to = 240, by = 30)) +
  labs(title = "Mode Split by Drive Time for Home-Based Other Trips", x = "Drive Time (mins)", y = "Mode Share (Fraction of Total Trips)", color = "Mode") +
  theme_minimal()
```

```{r mode-split-hbw-ubi, fig.cap='Figure 32. Mode Split by Drive Time for Home-Based Work Trips, in an Unconditional Cash Flow Scenario', out.width='80%', fig.asp=.75, fig.align='center'}
# HBW
ggplot()+
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_transit_HBW_UBI/n_total_HBW_UBI), color = "Transit"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_SOV_HBW_UBI/n_total_HBW_UBI), color = "SOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_HOV_HBW_UBI/n_total_HBW_UBI), color = "HOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  scale_x_continuous(breaks = seq(from =0, to = 240, by = 30)) +
  labs(title = "Mode Split by Drive Time for Home-Based Work Trips", x = "Drive Time (mins)", y = "Mode Share (Fraction of Total Trips)", color = "Mode") +
  theme_minimal()
```

```{r mode-split-nhb-ubi, fig.cap='Figure 33. Mode Split by Drive Time for Non-Home-Based Trips, in an Unconditional Cash Flow Scenario', out.width='80%', fig.asp=.75, fig.align='center'}
# NHB
ggplot()+
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_transit_NHB_UBI/n_total_NHB_UBI), color = "Transit"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_SOV_NHB_UBI/n_total_NHB_UBI), color = "SOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_HOV_NHB_UBI/n_total_NHB_UBI), color = "HOV"), alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  scale_x_continuous(breaks = seq(from =0, to = 240, by = 30)) +
  labs(title = "Mode Split by Drive Time for Non Home-Based Trips", x = "Drive Time (mins)", y = "Mode Share (Fraction of Total Trips)", color = "Mode") +
  theme_minimal()
...
When generating our one hour OD Matrix we decided to create a report illustrating trips during rush hour (5pm to 6pm). We believe rush hour will present greater trip volume than other times of day, as a result of a high share of commute-related trips occurring during that time frame. While we estimate that Monday at 5pm will have more travel than Saturday at 5pm, we assume that 5pm reflects a time of day when most travel occurs. Furthermore, HBW was 90% SOV driving, and we expect to see a large volume of cars during this selected time period. 
...
We chose the Bureau of Public Roads (BPR) delay function, because our preliminary research indicated that it is a common standard in travel demand modeling. Delay functions are mathematical formulas utilized by demand forecasting models' traffic allocation process to factor in the impact of heightened traffic volume on the time required to traverse all potential routes connecting diverse travel origins and destinations. These functions reflect the dynamic between traffic flow and travel time. As time is inversely proportional to the average speed of the traffic current, the delay functions are analogous to the flow-speed relations. Other functions that could be utilized are conical functions (CF), Akçik and Troutbeck function (ATF), and delay logistics function (LF). We chose the BPR delay function as it is one of the oldest and most widely used among the available delay functions.


#### Method

We acknowledge that there are inherent limits to route assignment choices. For our traffic assignment method, we chose stochastic equilibrium, which is the more widely-used deterministic user equilibrium assignment process. This approach considers the possibility of encountering uncertain travel times due to congested conditions, inaccuracies in information or perception, and navigating unfamiliar or new routes. Additionally, it has been suggested that accounting for stochastic variability, when adequately characterized, can provide a more accurate representation of the inherent day-to-day variability in travel times. We felt as though this was an apt method to capture the variability in travel. 
...
The congestion and resulting travel times were severely impacted by the sparseness of the road network and the longer connectors that are surrounded by many one-way roads. This led to some entirely unrealistic travel times of many days (Figure 34). As an example, Connector 1081745 has a length of 14.8 miles and an A-B travel time of 19,000 minutes and a B-A travel time of 59 minutes. If we were to extrapolate travel times from our model into the context of the real world the B-A travel time appears to be in the realm of possible travel times. However, A-B is clearly wildly inaccurate. This presents a clear limit to our model and has caused us to question how to proceed with the remaining assignments.

When visualizing our findings we chose to represent traffic congestion without the drastic outliers by filtering our flows to trips less than 240 minutes and to exclude connector roads. Additionally, we also filtered our traffic flows by road time to see where congestion was amassing. Overall, total VMT in the network is 24,150,645 miles, and total transit ridership in the system 2584.


```{r map-cong-time-all, fig.cap='Figure 34. Some Roads in Our Network are Unrealistically Congested, with Travel Times of More than 6 Hours', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(subset(TA_flows_geom, congestion_time > 240)) + 
  geom_sf(aes(color = congestion_time)) +
  scale_color_continuous(type = "gradient") + 
  labs(color = "Congestion Time (minutes)") +
  theme_void()
```


When visualizing our findings we chose to represent traffic congestion without the drastic outliers by filtering our flows to trips less than 240 minutes and to exclude connector roads (Figures 35-36). 


```{r map-cong-time-filter, fig.cap='Figure 35. Connectors are Often the Most Congested Roadways in the Network', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(subset(TA_flows_geom, congestion_time < 240)) + 
  geom_sf(aes(color = congestion_time)) +
  scale_color_continuous(type = "gradient") + 
  labs(color = "Congestion Time (minutes)") +
...
```{r map-cong-time-non-connector, fig.cap='Figure 36. Finding the Log of Congested Travel Times Reveals More Variation throughout the Network', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(subset(TA_flows_geom, Type != "")) + 
  geom_sf(aes(color = log(congestion_time))) +
  scale_color_continuous(type = "gradient") + 
  labs(color = "Log of Congestion Time") +
  theme_void()
```


Additionally, we also filtered our traffic flows by road type to see where congestion was amassing (Figures 37-38). Overall, total VMT in the network is 24,150,645 miles, and total transit ridership in the system 2584.


```{r map-non-connector-VMT-filter, fig.cap='Figure 37. When Filtering out Connector Roads, VMT is Greater on Long Peripheral Roads', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(subset(TA_flows_geom, Type != "")) + 
  geom_sf(aes(color = Tot_VMT)) +
  scale_color_continuous(type = "viridis", trans = "log") + 
  labs(color = "Vehicle Miles Traveled") +
  theme_void()
```

```{r map-non-connector-VMT, fig.cap='Figure 38. Visualizing VMT by Road Type Shows More Traffic on Connector Roads', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(TA_flows_geom) + 
  geom_sf(aes(color = Tot_VMT)) +
...
We held constant assumptions and methods from our Scenario A. The geographic distribution of VMT remains similar, even with the increased number of trips (Figure 39). 

```{r probability matrix UBI, echo=FALSE, include=FALSE}
prob_travel_matrix_UBI <- full_TTmatrix_UBI %>%
  select(from_GEOID, n_transit_total_UBI, n_SOV_total_UBI, n_HOV_total_UBI, n_total_total_UBI) %>%
  mutate(GEOID = as.character(from_GEOID)) %>%
  group_by(GEOID) %>%
  summarise(n_transit_total_UBI = sum(n_transit_total_UBI), 
            n_total_total_UBI = sum(n_total_total_UBI),
            n_SOV_total_UBI = sum(n_SOV_total_UBI),
            n_HOV_total_UBI = sum(n_HOV_total_UBI)) %>%
  mutate(pct_transit_UBI = (n_transit_total_UBI/n_total_total_UBI),
         pct_SOV_UBI = (n_SOV_total_UBI/n_total_total_UBI),
         pct_HOV_UBI = (n_HOV_total_UBI/n_total_total_UBI)) 
  
transit_prob_tracts_UBI <- tract_trip_info %>% 
  select(GEOID, geometry) %>% 
  left_join(prob_travel_matrix_UBI) %>%
  mutate(pct_transit_UBI = ifelse(pct_transit_UBI < 0.00000000001, NA, pct_transit_UBI))
```


```{r cars UBI, echo=FALSE, include=FALSE}
full_TTmatrix_UBI <- full_TTmatrix_UBI %>%
  mutate(n_HOV_HBO_veh_UBI = n_HOV_HBO_UBI / 2.71,
         n_HOV_HBW_veh_UBI = n_HOV_HBW_UBI / 2.42,
         n_HOV_NHB_veh_UBI = n_HOV_NHB_UBI / 2.75)
  
PA_matrix_UBI <- full_TTmatrix_UBI %>%
  mutate(HOV_flow_UBI = n_HOV_HBO_veh_UBI + n_HOV_HBW_veh_UBI + n_HOV_NHB_veh_UBI,
         SOV_flow_UBI = n_SOV_total_UBI) %>%
  mutate(flows_UBI = HOV_flow_UBI + SOV_flow_UBI) %>%
  select(from_GEOID, to_GEOID, flows_UBI)

TA_flows_UBI <- read_csv("okc_roads_prayerhand+ubi-LinkFl.csv")

TA_flows_geom_UBI <- road_geom %>%
  left_join(TA_flows_UBI)

#PA_matrix_transcad_UBI <- PA_matrix_UBI %>%
  #rowwise() %>%
  #mutate(from_node = get_node_id(from_GEOID),
        #to_node = get_node_id(to_GEOID)) %>%
  #select(from_node, to_node, from_GEOID, to_GEOID, flows_UBI)

#write.csv(PA_matrix_transcad_UBI, "PA_matrix_transcad_UBI.csv")
```

```{r map-non-connector-VMT-filter-ubi, fig.cap='Figure 39. When Filtering out Connector Roads, VMT is Greater on Long Peripheral Roads', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(subset(TA_flows_geom_UBI, Type != "")) + 
  geom_sf(aes(color = Tot_VMT)) +
  scale_color_continuous(type = "viridis", trans = "log") + 
  labs(color = "Vehicle Miles Traveled") +
  theme_void()
...
```{r df for TA flows without connectors, echo=FALSE}
TA_flows_NoCon <- subset(TA_flows, Type != "")

TA_flows_UBI_NoCon <- subset(TA_flows_UBI, Type != "")
```

```{r summary table, echo=FALSE, include=FALSE}
summary_table <- data.frame (matrix(NA, nrow = 9, ncol = 4))

rownames(summary_table) <- c("Total Trips", 
                             "Total HBO Trips",
                             "Total HBW Trips",
                             "Total NHB Trips",
                             "Total Trips by SOV",
                             "Total Trips by HOV",
                             "Total Trips by Transit",
                             "Total Vehicle Miles Traveled",
                             "Total Vehicle Miles Traveled (Excluding Connectors)")

colnames(summary_table) <- c ("Scenario A", "Scenario B", "Difference (Count)", "Difference (Percentage)")

## Enter values here
summary_table$`Scenario A`[1] <- sum(full_travel_matrix_with_flows$n_total_total) 
summary_table$`Scenario A`[2] <- sum(full_travel_matrix_with_flows$n_total_HBO)
summary_table$`Scenario A`[3] <- sum(full_travel_matrix_with_flows$n_total_HBW)
summary_table$`Scenario A`[4] <- sum(full_travel_matrix_with_flows$n_total_NHB)
summary_table$`Scenario A`[5] <- sum(full_travel_matrix_with_flows$n_SOV_total)
summary_table$`Scenario A`[6] <- sum(full_travel_matrix_with_flows$n_HOV_total)
summary_table$`Scenario A`[7] <- sum(full_travel_matrix_with_flows$n_transit_total)
summary_table$`Scenario A`[8] <- sum(TA_flows$Tot_VMT)
summary_table$`Scenario A`[9] <- sum(TA_flows_NoCon$Tot_VMT)
  
summary_table$`Scenario B`[1] <- sum(full_TTmatrix_UBI$n_total_total_UBI)
summary_table$`Scenario B`[2] <- sum(full_TTmatrix_UBI$n_total_HBO_UBI)#total HBO trips here
summary_table$`Scenario B`[3] <- sum(full_TTmatrix_UBI$n_total_HBW_UBI)#total HBW trips here
summary_table$`Scenario B`[4] <- sum(full_TTmatrix_UBI$n_total_NHB_UBI)#total NHB trips here
summary_table$`Scenario B`[5] <- sum(full_TTmatrix_UBI$n_SOV_total_UBI)#total SOV trips here
summary_table$`Scenario B`[6] <- sum(full_TTmatrix_UBI$n_HOV_total_UBI)#total HOV trips here
summary_table$`Scenario B`[7] <- sum(full_TTmatrix_UBI$n_transit_total_UBI)
summary_table$`Scenario B`[8] <- sum(TA_flows_UBI$Tot_VMT)
summary_table$`Scenario B`[9] <- sum(TA_flows_UBI_NoCon$Tot_VMT)
  
## Calculate differences
summary_table$`Difference (Count)`[1] <- summary_table$`Scenario B`[1]-summary_table$`Scenario A`[1]
summary_table$`Difference (Count)`[2] <- summary_table$`Scenario B`[2]-summary_table$`Scenario A`[2]
summary_table$`Difference (Count)`[3] <- summary_table$`Scenario B`[3]-summary_table$`Scenario A`[3]
summary_table$`Difference (Count)`[4] <- summary_table$`Scenario B`[4]-summary_table$`Scenario A`[4]
summary_table$`Difference (Count)`[5] <- summary_table$`Scenario B`[5]-summary_table$`Scenario A`[5]
summary_table$`Difference (Count)`[6] <- summary_table$`Scenario B`[6]-summary_table$`Scenario A`[6]
summary_table$`Difference (Count)`[7] <- summary_table$`Scenario B`[7]-summary_table$`Scenario A`[7]
summary_table$`Difference (Count)`[8] <- summary_table$`Scenario B`[8]-summary_table$`Scenario A`[8]
summary_table$`Difference (Count)`[9] <- summary_table$`Scenario B`[9]-summary_table$`Scenario A`[9]

## Calculate diff percentages
summary_table$`Difference (Percentage)`[1] <- ((summary_table$`Scenario B`[1]-summary_table$`Scenario A`[1])/summary_table$`Scenario A`[1])*100
summary_table$`Difference (Percentage)`[2] <- ((summary_table$`Scenario B`[2]-summary_table$`Scenario A`[2])/summary_table$`Scenario A`[2])*100
summary_table$`Difference (Percentage)`[3] <- ((summary_table$`Scenario B`[3]-summary_table$`Scenario A`[3])/summary_table$`Scenario A`[3])*100
summary_table$`Difference (Percentage)`[4] <- ((summary_table$`Scenario B`[4]-summary_table$`Scenario A`[4])/summary_table$`Scenario A`[4])*100
summary_table$`Difference (Percentage)`[5] <- ((summary_table$`Scenario B`[5]-summary_table$`Scenario A`[5])/summary_table$`Scenario A`[5])*100
summary_table$`Difference (Percentage)`[6] <- ((summary_table$`Scenario B`[6]-summary_table$`Scenario A`[6])/summary_table$`Scenario A`[6])*100
summary_table$`Difference (Percentage)`[7] <- ((summary_table$`Scenario B`[7]-summary_table$`Scenario A`[7])/summary_table$`Scenario A`[7])*100
summary_table$`Difference (Percentage)`[8] <- ((summary_table$`Scenario B`[8]-summary_table$`Scenario A`[8])/summary_table$`Scenario A`[8])*100
summary_table$`Difference (Percentage)`[9] <- ((summary_table$`Scenario B`[9]-summary_table$`Scenario A`[9])/summary_table$`Scenario A`[9])*100
```

```{r summary tale visualization}
kable(summary_table)
```


We observe that in our modeled unconditional cash transfer scenario, almost all metrics increase very slightly as a result of increasing the median income by $12,000. Total number of trips increased from 7,684,930 to 7,768,877 (about 84,000 trip or 1.1% increase). Trips went up across all purpose types. Non-home-based trip types increased about 0.46%, less than all other trip types, which increased by about 1%. Of the three trip types, home-based other trips increased by the highest percentage, which confirms our initial expectation. This increase reflects the fact that income has the strongest productive effect on home-based other trips, as observed in the NHTS data.  

Surprisingly, VMT decreased by about 500,000 miles, or 2%, compared to our original model. This is especially unexpected after seeing the increased number of trips generated by the hypothetical cash transfer. Following the sequence of the four-step travel demand model, the results of each step were in line with our expectations up to the final trip assignment stage. The output from that step, the table of vehicle flows per link, showed lower VMT relative to the original model's output. We excluded the connector roads from the link flow tables in both scenarios to compare VMT on all other roads in the study area. We suspected that connector roads may have been contributing a large number of VMT in both scenarios, and wanted to control for that influence. However, this analysis found that VMT still decreases in Scenario B, though by a slightly smaller percentage.  

One hypothesis for this finding relates to the higher concentrations of households below the poverty line in the center of the MSA. We are wondering, if lower income people are concentrated in the city center, would increasing their income result in an increase in trip numbers and decrease in VMT? The mixed results raise many questions for further research. 


# Discussion

When we set out to create this model, we were interested in understanding the travel externalities of unconditional cash transfers. Economic programs like UBI are not often discussed within the context of transportation planning, and we were curious to understand how changes to income could affect travel demand and behavior. We also wanted to explore the idea that higher incomes are related to more trips, and that the number of trips in a region is a potentially useful proxy for its vitality (economically, socially, etc).

While the utility of this specific model may be limited, further research should focus on the impact of cash transfers/universal basic income programs on low-income households in particular, to see how travel behavior changes with increased income. 

If time had allowed for further development of this model, we would want to better what happened to the VMT in our unconditional cash transfer scenario. We tried to systematically tease apart each step, but a more in depth and localized model would be a more sound approach to understanding what resulted in the combined trip number increase and VMT decrease. Additionally, more time spent on thinking through the structure of an unconditional cash transfer program would be beneficial to the realism of the modeled scenario. 
 
One limitation of analyzing an unconditional cash transfer program within the context of a travel demand model is the need to grapple with a variety of assumptions at the outset. We made a number of assumptions about household characteristics (e.g., vehicle ownership, employment) by asserting that they would remain constant in our Scenario B. We recognize that this is unrealistic, but would need significantly more research to capture the full impact of the program on other behaviors that could influence travel demand. 
...
**Tot_VMT** - Total vehicle miles traveled along each link in the road network, as determined by TransCAD analysis. 
...
* Mahdi, Mohammed Bally, et al. “Compatibility between Delay Functions and Highway Capacity Manual on Iraqi Highways.” De Gruyter, De Gruyter Open Access, 1 Jan. 2022, https://www.degruyter.com/document/doi/10.1515/eng-2022-0022/html?lang=en.
* “Travel Forecasting Resource.” TF Resource, https://tfresource.org/topics/Stochastic_Traffic_Assignment.html. 
