...
To test this we changed median income to reflect the $12,000 cash transfer, reran a linear regression in trip generation stage, generated a new number of productions and attractions, and compared to number of trips (and share of trips by purpose) in original model (we performed this same comparison for the number of VMT), finally, we reran trip distribution, mode choice, trip assignment.  
...
The Oklahoma City MSA is predominantly white, with 64.3% of residents identifying as white alone, followed by 13.3% Hispanic or Latino, 10.1% Black, 3.3% American Indian and Alaska Native, 3.1% Asian, 0.1% Native Hawaiian and Pacific Islander. 36.8% of residents in the study are are ages 35-64, 24.7% are under the age of 18, 25.0% are ages 18-34, and 13.6% are ages 65 and older. The median household income in 2021 was $59,084 (Figure 1) – higher than the statewide average and lower than the national average. About 13.9% of residents live below the poverty line (Figure 2) – lower than the statewide average and higher than the national average.

```{r poverty-line, echo=FALSE, warning=FALSE, fig.cap='Figure 2. Households Living Below the Federal Poverty Line are Concentrated Near Downtown Oklahoma City', out.width='80%', fig.asp=.75, fig.align='center'}
...
* We assume that employment exists in three categories (Figure 9):
    + Retail
    + Service
    + Basic
...
```{r employment-county, echo=FALSE, fig.cap='Figure 9. Oklahoma County has the Most Employees in the MSA, and the Majority Are Service Sector Workers', out.width='80%', fig.asp=.75, fig.align='center'}
...
OKC generally follows a monocentric pattern of development with the greatest density of people living and working in downtown Oklahoma City (Figures 10-11), located in the center of the MSA.
...
```{r emp-density, echo=FALSE, fig.cap='Figure 10. Population Density is Greatest in the Center of the Oklahoma City MSA', out.width='80%', fig.asp=.75, fig.align='center'}
...
```{r pop-density, echo=FALSE, fig.cap='Figure 11. Job Density is Also Concentrated in the Center of the Oklahoma City MSA', out.width='80%', fig.asp=.75, fig.align='center'}
...
``` {r access-indices, echo=FALSE, fig.cap='Figure 12. The Transit Hub South of Downtown Oklahoma City is A Recognizable Blip on this Plot Showing Car and Transit Accessibility Plotted Against One Another', out.width='80%', fig.asp=.75, fig.align='center'}
...
```{r access-histogram, echo=FALSE, fig.cap='Figure 13. Nearly Two-Thirds of Zones Do Not Have Any Transit Accessibility', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(accessibility_summary) +
...
```{r spatial-access-transit, echo=FALSE, warning=FALSE, fig.cap='Figure 15. Accessibility by Transit is Concentrated in the Downtown Area, with Some Accessibility around a Southern Transit Hub in Norman', out.width='80%', fig.asp=.75, fig.align='center'}
...
```{r centroids_zones, echo=FALSE, fig.cap='Figure 4. Components of our Model: Zones and their Centroids', out.width='80%', fig.asp=.75, fig.align='center'}
county_geom <- full_tract_info %>%
    separate(NAME, into = c("tract-num", "county", "state"), sep = ", ") %>%
    group_by(county) %>%
    summarize(geometry = st_union(geometry))
...
```{r roads, echo=FALSE, fig.cap='Figure 5. Components of our Model:Roads, Centroids, Centroid Connectors, and Transit Lines', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot() +
  annotation_map_tile(zoomin = 0, progress = "none", type = "cartolight") +
...
```{r transit, echo=FALSE, fig.cap='Figure 6. Components of our Model:Roads, Centroids, Centroid Connectors, and Transit Lines', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot() +
  annotation_map_tile(zoomin = 0, progress = "none", type = "cartolight") +
  geom_sf(data = full_tract_info, aes(), fill = NA, color="darkgrey", size=0.1) +
  geom_sf(data = county_geom, aes(), fill = NA, color="black", size=0.5) +
...
The charts below illustrate how the timing of various transit trips break down across walk, wait, transfer and travel time (Figures 7-8).


```{r stacked-bar-chart, echo=FALSE, warning=FALSE, fig.cap='Figures 7 and 8. Travel Time Makes Up the Bulk of Most Trips, but Transfer Wait Time is Also A Large Portion of Trip Duration', out.width='80%', fig.asp=.75, fig.align='center'}
...
As expected, productions and attractions are primarily concentrated in the downtown area—attractions even more so than productions (Figures 16-17). Both generally follow the density maps of residences and employment that we created earlier in the semester.


```{r density-prod, echo=FALSE, fig.cap='Figure 16. Trip Productions are Primarily Concentrated in the Downtown Area', out.width='80%', fig.asp=.75, fig.align='center'}
...
```{r density-attr, echo=FALSE, fig.cap='Figure 17. Trip Attractions are Even More Concentrated in the Downtown Area', out.width='80%', fig.asp=.75, fig.align='center'}
#attractions
...
```{r hbo-chord, echo = FALSE, fig.cap='Figure 18. Intra- and Inter-County Flows for Home-Based Other Trips', fig.align='center'}
...
```{r hbw-chord, echo = FALSE, fig.cap='Figure 19. Intra- and Inter-County Flows for Home-Based Work Trips', fig.align='center'}
...
```{r nhb-chord, echo = FALSE, fig.cap='Figure 20. Intra- and Inter-County Flows for Non-Home-Based Trips', fig.align='center'}
...
Compared to the number of trip productions generated in the model reflecting existing conditions, the model with an unconditional cash transfer had slightly greater trip productions for all trip types. The tracts with the greatest increases in trip productions are not particularly geographically concentrated. 


```{r plot hbo difference, echo=FALSE, fig.cap='Figure XX. Difference in Number of Home-Based Other Trip Productions in Scenario B compared to Scenario A', out.width='80%', fig.asp=.75, fig.align='center'}
...
```{r plot hbw difference, echo=FALSE, fig.cap='Figure XX. Difference in Number of Home-Based Work Trip Productions in Scenario B compared to Scenario A', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(difference_df) +
...
```{r plot nhb difference, echo=FALSE, fig.cap='Figure XX. Difference in Number of Non-Home-Based Trip Productions in Scenario B compared to Scenario A', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(difference_df) +
...
```{r density-prod UBI, echo=FALSE, fig.cap='Figure 16. Trip Productions are Primarily Concentrated in the Downtown Area', out.width='80%', fig.asp=.75, fig.align='center'}
#productions dot density map
hbo_prod_pts <- st_sample(tract_trip_info_UBI, 
                              size = ceiling(tract_trip_info_UBI$hbo_trip_prod/1000))
 
hbo_prod_df <- tibble(trip_type = rep("Home-Based Other",
                                            length(hbo_prod_pts))) %>%
  st_sf(geom = hbo_prod_pts)

hbw_prod_pts <- st_sample(tract_trip_info_UBI,
                               size = ceiling(tract_trip_info_UBI$hbw_trip_prod/1000))

hbw_prod_df <- tibble(trip_type = rep("Home-Based Work", 
                                              length(hbw_prod_pts))) %>%
  st_sf(geom = hbw_prod_pts)

nhb_prod_pts <- st_sample(tract_trip_info_UBI,
                               size = ceiling(tract_trip_info_UBI$nhb_trip_prod/1000))
...
```{r density-attr UBI, echo=FALSE, fig.cap='Figure 17. Trip Attractions are Even More Concentrated in the Downtown Area', out.width='80%', fig.asp=.75, fig.align='center'}
#attractions
hbo_att_pts <- st_sample(tract_trip_info_UBI, 
                              size = ceiling(tract_trip_info_UBI$hbo_trip_attr/1000))
 
hbo_att_df <- tibble(trip_type = rep("Home-Based Other",
                                            length(hbo_att_pts))) %>%
  st_sf(geom = hbo_att_pts)

hbw_att_pts <- st_sample(tract_trip_info_UBI,
                               size = ceiling(tract_trip_info_UBI$hbw_trip_attr/1000))

hbw_att_df <- tibble(trip_type = rep("Home-Based Work", 
                                              length(hbw_att_pts))) %>%
  st_sf(geom = hbw_att_pts)

nhb_att_pts <- st_sample(tract_trip_info_UBI,
                               size = ceiling(tract_trip_info_UBI$nhb_trip_attr/1000))
...
Trip flows within and between counties remained fairly consistent across the two scenarios. 
...
```{r hbo-chord-UBI, echo = FALSE, fig.cap='Figure 18. Intra- and Inter-County Flows for Home-Based Other Trips', fig.align='center'}
...
```{r hbw-chord-UBI, echo = FALSE, fig.cap='Figure 19. Intra- and Inter-County Flows for Home-Based Work Trips', fig.align='center'}
...
```{r nhb-chord-UBI, echo = FALSE, fig.cap='Figure 20. Intra- and Inter-County Flows for Non-Home-Based Trips', fig.align='center'}
...
In order to understand the relationship between trip length and mode choice, we plotted the percent of trips using a particular mode (as predicted by our model) against the drive time of that trip (Figure 21). We then further subset the data by purpose to illustrate the difference in mode share for different kinds of trips (Figures 22-24). 
...
```{r mode-split-all, fig.cap='Figure 21. Mode Split by Drive Time for all Trip Purposes', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot()+
...
```{r mode-split-hbo, fig.cap='Figure 22. Mode Split by Drive Time for Home-Based Other Trips', out.width='80%', fig.asp=.75, fig.align='center'}
# HBO
ggplot()+
...
```{r mode-split-hbw, fig.cap='Figure 23. Mode Split by Drive Time for Home-Based Work Trips', out.width='80%', fig.asp=.75, fig.align='center'}
# HBW
ggplot()+
...
```{r mode-split-nhb, fig.cap='Figure 24. Mode Split by Drive Time for Non-Home-Based Trips', out.width='80%', fig.asp=.75, fig.align='center'}
# NHB
ggplot()+
...
We started by assigning the coefficients from our Scenario A mode choice model to the Scenario B mode choice model, and then calibrated from there. 

The coefficients for home-based work and non-home-based trips resulted in mode share splits within a 5 percentage point threshold of the observed mode share by purpose data from the NHTS. With a small amount of calibration, we found that the coefficient for non-home-based trips decreased slightly from -0.

******

HBO_UBI   -0.18 (Same as existing conditions)
HBW_UBI  -0.06 (Same as existing conditions)
NHB_UBI   -0.19 
...
```{r model_2_UBI, echo=FALSE, include=FALSE}
# CONSTANTS
...
model_compare_UBI <- rbind(modeled_mode_by_purpose_2, modeled_mode_by_purpose_UBI) %>%
  arrange(purpose) 
...
This confirms our expectation that mode choice isn't significantly affected by an additional influx of income. 
...
```{r mode-split-all-ubi, fig.cap='Figure 21. Mode Split by Drive Time for all Trip Purposes', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot()+
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_transit_total_UBI/n_total_total_UBI)), color = "red", alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_SOV_total_UBI/n_total_total_UBI)), color = "blue", alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_HOV_total_UBI/n_total_total_UBI)), color = "goldenrod", alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  scale_x_continuous(breaks = seq(from =0, to = 240, by = 30)) +
  labs(x = "Drive Time (mins)", y = "Mode Share (Fraction of Total Trips)") +
  theme_minimal()
```

```{r mode-split-hbo-ubi, fig.cap='Figure 22. Mode Split by Drive Time for Home-Based Other Trips', out.width='80%', fig.asp=.75, fig.align='center'}
# HBO
ggplot()+
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_transit_HBO_UBI/n_total_HBO_UBI)), color = "red", alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_SOV_HBO_UBI/n_total_HBO_UBI)), color = "blue", alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  stat_summary_bin(data = full_TTmatrix_UBI, aes(x = drive_time, y = (n_HOV_HBO_UBI/n_total_HBO_UBI)), color = "goldenrod", alpha = 0.5, fun="mean", bins=84, size=2, geom="point") +
  scale_x_continuous(breaks = seq(from =0, to = 240, by = 30)) +
  labs(title = "Mode Split by Drive Time for Home-Based Other Trips", x = "Drive Time (mins)", y = "Mode Share (Fraction of Total Trips)") +
  theme_minimal()
```
...
```{r map-cong-time-all, fig.cap='Figure 25. Some Roads in Our Network are Unrealistically Congested, with Travel Times of More than 6 Hours', out.width='80%', fig.asp=.75, fig.align='center'}
...
When visualizing our findings we chose to represent traffic congestion without the drastic outliers by filtering our flows to trips less than 240 minutes and to exclude connector roads (Figures 26-27). 


```{r map-cong-time-filter, fig.cap='Figure 26. Connectors are Often the Most Congested Roadways in the Network', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(subset(TA_flows_geom, congestion_time < 240)) + 
  geom_sf(aes(color = congestion_time)) +
  scale_color_continuous(type = "gradient") + 
  theme_void()
...
```{r map-cong-time-non-connector, fig.cap='Figure 27. Finding the Log of Congested Travel Times Reveals More Variation throughout the Network', out.width='80%', fig.asp=.75, fig.align='center'}
...
Additionally, we also filtered our traffic flows by road type to see where congestion was amassing (Figures 28-29). Overall, total VMT in the network is 24,150,645 miles, and total transit ridership in the system 2584.


```{r map-non-connector-VMT-filter, fig.cap='Figure 28. When Filtering out Connector Roads, VMT is Greater on Long Peripheral Roads', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(subset(TA_flows_geom, Type != "")) + 
...
```{r map-non-connector-VMT, fig.cap='Figure 29. Visualizing VMT by Road Type Shows More Traffic on Connector Roads', out.width='80%', fig.asp=.75, fig.align='center'}
ggplot(TA_flows_geom) + 
  geom_sf(aes(color = Tot_VMT)) +
...
Total number of trips increased from 7,684,930 to 7,768,877 (about 84,000 trip or 1.1% increase). By purpose: Trips went up across all purpose types: HBO trips, HBW trips, NHB trips.Total VMT decreased from 24,150,645 to 23,650,677 (about 500,000 miles less or 2.1% decrease). 


# Analysis + Discussion

[opening sentence to come]

Mode choice: if home-based other has the highest share of HOV trips AND HOV trips is the most common mode choice for home-based other trips. AND home-based other trips saw the largest percentage increase as a result of our UCT. Then it makes sense that the share of HOV trips should increase overall.

We have observed higher concentrations of households below the poverty line in the center of the MSA. We are wondering, if lower income people are concentrated in the city center, would increasing their income result in an increase in trip numbers and the least possible change in VMT? 


What can decision makers learn?
  - Further research should focus on the impact of cash transfers/UBI on those households in particular to see how travel behavior changes with increased income.
  - What did we want to learn / what do we think if done in another context could be learned?
  - UBI and other economic programs do not fall within the realm of travel -- interesting to understand the cobenefits/nonbenefits
  - Relates to nexus of higher income = higher trips; trip number is potentially useful proxy to vitality of a region (economically, socially, etc)
  - Travel externalities of unconditional cash transfers 
  
What important questions have not been addressed?
  - What happened to the VMT ?!?!?!
  - We tried to systematically tease apart each step -- a more in depth and localized model would be a more sound approach.
  - Address the way in which a cash transfer program was structured. And the effects of structuring it in a different way.

What What important questions cannot been addressed from an analysis like this?
  - We make a lot of assumptions of things that don't change, which is an unrealistic approach. An analysis of cash transfer impact on travel behavior would need to make assumptions of things that would change or else it would not account for these changes (i.e., modeling what this would look like and assume that more individuals would buy cars, or have all else remain constant and miss an important behavioral change that could influence results).
