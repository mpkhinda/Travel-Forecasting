...
We compared mode split by trip type to the same observed data from NHTS as we used in Scenario A. 

```{r mode by purpose for UBI scenario, echo=FALSE}
kable(mode_by_purpose)
```
...
We started by assigning the coefficients from our Scenario A mode choice model to the Scenario B mode choice model. We applied the same standard for Scenario B, as well, aiming to have any discrepancies between the model and observed data from NHTS below a 5 percentage point threshold. 
...
We held constant assumptions and methods from our Scenario A. Overall, we were more interested VMT and total trips than we were in congestion, and were surprised that our model demonstrated less overall VMT despite and increase in total trips. 
...
Total number of trips increased from 7,684,930 to 7,768,877 (about 84,000 trip or 1.1% increase). Trips went up across all purpose types.

Total VMT decreased from 24,150,645 to 23,650,677 (about 500,000 miles less or 2.1% decrease). 

```{r summary table, echo=FALSE, include=FALSE}
summary_table <- data.frame (matrix(NA, nrow = 8, ncol = 4))
...
The results of our model for an unconditional cash transfer scenario are mixed and leave many questions unanswered. The juxtaposition of increased trip numbers and decreased VMT is surprising and counterintuitive. We have observed higher concentrations of households below the poverty line in the center of the MSA. We are wondering, if lower income people are concentrated in the city center, would increasing their income result in an increase in trip numbers and the least possible change in VMT? 

One possible explanation for the relatively high increase in HOV trips could be related to mode choice across the various trip types: home-based other trips saw the largest percentage increase as a result of our unconditional cash transfer. Among all home-based other trips, HOV is the most common mode choice. Therefore, it makes sense that the share of HOV trips should increase overall relative to other modes.
