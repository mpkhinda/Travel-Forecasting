...
full_centroid_geom <- geojson_sf("full_centroid_geom.geojson")
 # rowwise() %>%
#  mutate(node_ID = get_node_id(GEOID)) %>%
 # st_write("full_centroid_geom_GEOID.geojson")
...
We also created three chord diagrams showing the intra- and inter-county flows by trip purpose (Figures 18-20). This allows us to better understand which counties are more attractive or productive for each trip purpose. For example, we noticed that in Cleveland County (southeast of downtown), home-based other and non-home-based trips are more than half of all trips into the county, while home-based work trips are a smaller share. For all trip types with destinations in Cleveland County, Oklahoma County (centrally located) was a major producer, providing yet another illustration of the close relationship between these two central areas in the MSA.
...
```{r plot hbo difference, echo=FALSE}
ggplot(difference_df) +
  geom_sf(aes(fill=diff_hbo))
```

```{r plot hbw difference, echo=FALSE}
ggplot(difference_df) +
  geom_sf(aes(fill=diff_hbw))
```

```{r plot nhb difference, echo=FALSE}
ggplot(difference_df) +
...
```{r write csv with travel costs included UBI, include=FALSE}
# output new csv with distance info
#write_csv(full_travel_matrix, "full_travel_matrix.csv")
```
...
```{r sum VMT, echo=FALSE, include=FALSE}
#sum(TA_flows_geom$Tot_VMT)
```

```{r total transit ridership, echo=FALSE, include=FALSE}
...
Working theory: if lower income people are concentrated in the city center, increasing their income would have the greatest change of trip umbers with the least possible change in VMT. We have observed higher concentrations of households below the poverty line in the center of the MSA. 
...
## Scenario B: Effects of a Universal Basic Income in Oklahoma City
...
```{r load data UBI, echo=FALSE, include=FALSE}
# load travel matrix and geom
#full_travel_matrix_UBI <- read_csv("full_travel_matrix.csv")
#tract_centroid_geom <- geojson_sf("full_centroid_geom.geojson")
#tract_flows <- read_csv("tract_flows.csv")
#tract_trip_info <- geojson_sf("tract_trip_info.geojson")
...
#full_travel_matrix_UBI <- full_travel_matrix_with_flows %>%
 # mutate(drive_cost = drive_miles*IRS_mileage_rate)

# split costs for carpool
#full_travel_matrix_UBI <- full_travel_matrix_UBI %>%
 # mutate(carpool_cost_hbo = drive_cost / 2.71,
  #       carpool_cost_hbw = drive_cost / 2.42,
   #      carpool_cost_nhb = drive_cost / 2.75)

#create tract flows with UBI data
#tract_flows_UBI <- full_TTmatrix_UBI %>%
  #select(from_GEOID, to_GEOID, HBO_flow_UBI, HBW_flow_UBI, NHB_flow_UBI)

#join flows to ttmatrix
#full_TTmatrix_UBI <- full_TTmatrix_UBI %>%
 # mutate(from_GEOID = as.character(from_GEOID), to_GEOID = as.character(to_GEOID)) %>%
  #left_join(tract_flows_UBI)
...
```{r read in updated csv, echo=FALSE, include=FALSE}
...
```{r found sum of total vmt for each scenario, echo=FALSE, include=FALSE}
sum(TA_flows_geom$Tot_VMT)
sum(TA_flows_geom_UBI$Tot_VMT)
```

```{r found sum of total trips for each scenario, echo=FALSE, include=FALSE}
sum(full_travel_matrix$n_total_total)
sum(full_TTmatrix_UBI$n_total_total_UBI)
```

```{r found sum of total trips by type, echo=FALSE, include=FALSE}
sum(full_travel_matrix_with_flows$n_total_HBO)
sum(full_TTmatrix_UBI$n_total_HBO_UBI)

sum(full_travel_matrix_with_flows$n_total_HBW)
sum(full_TTmatrix_UBI$n_total_HBW_UBI)

sum(full_travel_matrix_with_flows$n_total_NHB)
sum(full_TTmatrix_UBI$n_total_NHB_UBI)
```
