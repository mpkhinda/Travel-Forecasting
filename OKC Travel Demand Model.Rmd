...
```{r isochrones, echo=FALSE}
iso_origin <- 848988

TTmatrix_from_origin <- TTmatrix_long %>%
  filter(origin == iso_origin)

iso_drivetime_from_origin <- ok_msa_w_centroids %>%
  select(GEOID, NAME, geometry, centroid_id) %>%
  left_join(TTmatrix_from_origin, by = c("centroid_id" = "destination"))
...
ggplot(iso_drivetime_from_origin) + 
  ...
  scale_fill_gradient2(low = "white", mid = "thistle", high = "orchid4",
                       midpoint = median(iso_drivetime_from_origin$travel_time_mins),
                       name = "Transit Travel\ntime to all\nzones from centroid 848988\n(minutes)") +
