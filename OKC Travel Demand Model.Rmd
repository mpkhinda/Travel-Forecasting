...
okc_matrix_roads_path <- "../Travel-Forecasting/TTmatrix_skim10.csv"
  
okc_matrix_roads <- read.csv(okc_matrix_roads_path)

okc_nodes_centroids_path <- "../Travel-Forecasting/nodes_okc_roads_prayerhands_withcentroids.csv"

okc_nodes_centroids <- read.csv(okc_nodes_centroids_path) %>%
  drop_na(centroid) %>%
  mutate(long = Longitude / 1000000) %>%
  mutate(lat = Latitude / 1000000)

okc_centroids_geom <- st_as_sf(okc_nodes_centroids, 
                                     coords=c("long", "lat"), 
                                     crs = 4326,
                                     agr = "constant") %>%
                      select(ID, geometry)
...
ok_msa_w_centroids$centroid_id <-  apply(st_intersects(ok_msa, okc_centroids_geom, sparse = FALSE), MARGIN = 2, 
               function(col) { 
                  okc_centroids_geom[which(col), ]$ID
               })

TTmatrix_long <- okc_matrix_roads %>%
   pivot_longer(
     cols = starts_with("X"),
...
ggplot(iso_drivetime_from_origin) + 
  ...
  scale_fill_gradient2(low = "white", mid = "thistle", high = "orchid4",
                       ...
                       name = paste("Transit Travel\ntime to all\nzones from", iso_origin, "\n(minutes)")) +
  geom_sf(data = iso_origin_geom, shape = 19, size = 3)
}

isochrone_generator(848988)
```

```{r test, echo=FALSE}
isochrone_generator(848782)

isochrone_generator(849063)
```
...
accessibility_matrix <- TTmatrix_long %>%
  filter(travel_time_mins<30) %>%
  group_by(origin) %>%
  tally() %>%
  rename(score = n)

accessibility_score_geom <- ok_msa_w_centroids %>%
  select(GEOID, NAME, geometry, centroid_id) %>%
  left_join(accessibility_matrix, by = c("centroid_id" = "origin"))

ggplot(accessibility_score_geom) + 
